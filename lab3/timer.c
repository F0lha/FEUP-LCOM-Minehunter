...
int timer_unsubscribe_int() {
	return (sys_irqrmpolicy(&hook_id)||sys_irqdisable(&hook_id));
}
	...
	hook_id = timer_subscribe_int();
	 ...
	 while( loops!=time ) {
	    ...
	    if (is_ipc_notify(ipc_status)) {
	    	printf("received noti\n");
	    	/* received notification */
	        switch (_ENDPOINT_P(msg.m_source)) {
	            case HARDWARE: /* hardware interrupt notification */
	            	printf("case hardware");
	                if (msg.NOTIFY_ARG & hook_id) {
	                	if(global_counter==60){
	                		loops++;
	                		global_counter=0;
	                	}
	                	else timer_int_handler();
	                }
	                break;
	            default:
